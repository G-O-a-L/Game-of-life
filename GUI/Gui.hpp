...
class GUI{
 ...
 public:
    ...
    void RenderWindow(){
        ...
        if(to_render.empty()){throw std::runtime_error("Invalid array, can't display nothing");}
    ...
    sf::Clock clock; //Créer un objet qui va suivre le temps
     ...
     while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if(bstop){ // Si bstop = true donc si le jeu est en pause
             ...
             if (next.handleEvent(event, window)) {
                gameLogic.nextStep(); // Itération suivante
                RenderWindow()
              }
        ...
        if(!bstop){ // Si l'on n'est pas en pause
             ...
             gameLogic.nextStep(); // à la fin du délai, on passe à l'itération suivante
             RenderWindow();
             clock.restart(); //Reset le compteur
             }
            }
