...
GameLogic::GameLogic(vector<int> cells, int x, int y) : Grid(cells, x, y) {}

// Method to calculate the next step in the game
void GameLogic::nextStep(const bool mode) {
    int xmax, ymax; // Initialize variables for the next generation
    std::vector<std::vector<Cell>> current_tab = getTab(); // Get the current state of the grid
    std::vector<std::vector<Cell>> next_tab = getTab(); // Create a copy of the current grid for the next generation
    ...
    if (mode == 1) {
        ...
        if (checkIfSame()) {
            ...
            std::cout << "Fin du programme, les deux dernière étapes sont identiques" << std::endl;
            return;
        }
    }
